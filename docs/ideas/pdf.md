...
状态（2026-09-15 起稿）：~~第一期已实现~~ **第一期已随 v0.11.0 发布（2026-09-16）**——桌面验证与随后的顶栏/跳转修正见「第一期桌面验证」；spike 已通过。用户 2026-09-15 拍板：引擎走**纯 Rust 栅格化（hayro）**、**第一期只读**、书架 EPUB/PDF 混排、**先跑 spike 再开第一期**、**无 outline 就不给目录**、样本 PDF 由用户放仓库旁不入库；pdf.js 与 pdfium 两条更重的路线留作后续（见「为什么不是 B / C」）。
...
### 跳转改为按页号（2026-09-16，用户反馈「pdf 应该按页跳转」）

上面第 2 条修好之后「全书 N%」能点到了，但它对 PDF 本来就是错位的坐标：`chapterChars` 每页 1，百分比只是页号的另一种写法，读者还得自己把 50% 换算成第 477 页。现在：

- PDF 的进度区给的是 **「· 跳页」**：浮层标题「跳到第 N 页」、单位「页」、预填当前页号，回车即把该页滚到视口顶部（`jumpToPdfPage` → `pdfGoTo`）。越界夹到 `[1, M]`，非数字不跳**且浮层不关**（留着改）。`inputMode="numeric"`。
- **EPUB 一字未改**：仍是「全书 N%」（各章字符数不等，百分比才是跨越章节的通用坐标）。`jumpToBookPercent` 去掉 PDF 分支后只剩 EPUB 路径。
- 顶栏文字不变，仍是「第 N / M 页（· 跨页）」——PDF 不再显示「全书 N%」，它与页号重复，而进度区横向空间本来就紧。
- 实测：在第 1 页点「· 跳页」→ 清空输入 500 →「第 500 / 952 页 · 跨页 · 跳页」；输入 1500 → 夹到 952；输入 `abc` → 不跳、浮层留着。
...
- 跳页：进度区「· 跳页」输入页号落到该页（越界夹到首/末页、非数字不跳且浮层不关）。EPUB 那边仍是「全书 N%」输入 0–100。
