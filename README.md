...
- **书架：** 启动进入本地书架（无 `ICED_READER_OPEN` 时）。列出 `data/library/` 里的 `.epub` 与 `.pdf`（书名、作者、封面、章节进度、质量；最近读过的在前，未读的按质量排序）。点封面继续读。封面右下角三点菜单：可「编辑元数据…」（见下条）或「从书库删除」——删除前会弹确认框，确认后连同这本书的进度与划线一起清除。阅读顶栏「书架」返回。首次导入会分析排版质量（界面显示进行中提示）：封面左上角标 优/良/中（悬停看理由——评分按「同书异版」对照：注文、像样的插图、无乱码、有作者、书内校验过的 ISBN 都算长处，未读排序里把更优的版本抬到前面）；若与书库中已有书为同一本（重打包或同版），提示「同书 ×N」、浮层对照两版差别，不自动合并。网格上方有轻量统计行（共 N 本 · 未读 · 已打开 · 无法打开）。尚无分类、子目录。
...
- **PDF（第一期）：** PDF 在 Rust 里逐页栅格化（`hayro`，纯 Rust，**不加任何 DLL**），阅读面是 **SumatraPDF 式的连续纸带**（不是 EPUB 的分栏分页器）：一列纸纵向滚动、滚轮就是滚动。**「适应宽度」= 纸宽实时贴合窗口宽**（拖动窗口立刻跟着变，此模式恒单页）；**「适应页面」= 整页贴合窗口高**，并可切「单页 / 双页 / 自动」——双页是书式配对（第 1 页单独，之后 2-3 / 4-5），够宽时「自动」会自己并排。←/→ 翻页、PageDown/空格滚一屏、Home/End 首末页；页码与进度跟着**视口里面积最大的那页**，目录跳页把目标页滚到顶部。相邻页在后台预取（实测命中后 0.1–0.6 ms），栅格是无损 WebP + 内存缓存，不写全书磁盘缓存。打开时会体检「**可见**文字有没有用未嵌入的字体」——扫描书（含带隐藏 OCR 文字层的）与正常嵌入字体的书都不会误报，只有真会缺字的书才提示。**PDF 有自己的质量角标**：真文字 + 字体全嵌入 + 有书签目录 = 优；扫描但带 OCR 文字层（可搜）= 良；纯扫描无可提取文字 = 中（悬停看理由）。暂不支持：选中文字、划线/备注、搜索、加密 PDF、替换 PDF 字体。
...
```powershell
Copy-Item target\release\IcedReader.exe target\release\IcedReader-0.10.2-windows-x64.exe
```
