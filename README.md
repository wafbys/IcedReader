...
- **窗口：** 默认 1120×780，最小 800×520。窗口标题为 `IcedReader {版本} ({git hash})`，如 `IcedReader 0.5.0 (8df0568)`——hash 是 build 时固化的 git 短 hash，方便对发布件溯源（无 git 环境构建时只显示版本号）。关闭时记住位置、大小和是否最大化（`data/window.json`），下次打开还原；全屏不记住。顶栏固定 52px；窄窗口或长书名用省略号，不换行撑高。阅读时窗口过窄（≤1180px）顶栏把低频按钮（打开 EPUB / 目录 / 划线 / 字体 / 全屏）收进右上「⋯」菜单，书架、上一章/下一章、字号、书名保持常显。
...
```powershell
Copy-Item target\release\iced-reader.exe target\release\IcedReader-0.5.0-windows-x64.exe
```
