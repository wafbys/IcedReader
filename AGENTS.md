...
④ **书内词注（内容呈现，非皮肤）：** 有些 EPUB（如微信读书导出的古籍）把词注塞在空 `<span data-wr-footernote="…">` 属性里，正文没有任何可见入口。`crates/formats-epub` 在 `chapter_html` 输出时把它们原位展开成带 `title` 的空 `<a class="wr-note" href="#wr-note-N">` 注标（序号由 CSS `::after` 画，**不新增文本节点**，保划线锚定稳定），并在所属段落后生成 `id="wr-note-N"` 的 `div.wr-notes` 注文块（悬停原生 tooltip 即读全文，点击注标走既有同文件锚点跳转到注块；其余注文形态不动）。父页可注入带 `id="iced-reader-note-style"` 的样式，只含 `a.wr-note`（注标小上标色）与 `.wr-notes`（段后注块的小字灰色布局）；父页不得增删正文内容、不得给注标补文本。转换只针对 `data-wr-footernote` 属性，别的书零影响。
...
改词注时：打开含 `data-wr-footernote` 词注的书（如仓库旁未提交的 `资治通鉴.epub`，可跑 `cargo test -p iced-reader-epub -- --ignored word_notes_expand_in_zztj` 做无桌面验证）。被注词后出现淡棕小上标序号（CSS 画的，正文没有加字）；悬停弹原生 tooltip 且能读完注文；点击跳到本段后注文块；翻页/字号变化后注标仍在原句；无词注的书（`fixtures/sample.epub`）正文不变；章节 iframe 仍未开脚本、没有残留 `data-wr-footernote`。
