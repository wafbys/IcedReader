...
格式层按 `Book` + `BookOpener` 抽象（当前只有 EPUB 适配器 `crates/formats-epub`），解析/排版逻辑不进 JS；前端命令形状尽量不动。
...
③ **用户划线高亮（用户主动标注，非皮肤）：** 允许父页在章节 doc 里注入带 `id="iced-reader-highlight-style"` 的样式，只含按颜色 key 列出的 `::highlight(iced-reader-highlight-{color})` 规则（现 `yellow`/`green`，key 即存储值）的背景色与 `color: inherit`，并用 CSS Custom Highlight API 上色；**不改 DOM 结构、不插 `mark`、不影响版式与内容**。选区捕获、点中判定、注入、删除全部由父页脚本完成（章节 iframe 仍不开 `allow-scripts`）。划线锚定按章内全局文本节点序号 + 节点内偏移（加原文摘录兜底），存 `data/annotations.json`（记录含 `color` 与 `pos`）。划线**下笔即定**：颜色在划线时选定，语义 opinionated——黄=重点（默认，含思考/存疑等一切有想法的标注）、绿=摘抄（纯摘录）；不做改色、不做范围调整（要换即删除重划，界面只有「划线（选色）」与「删除划线」，无橡皮擦式精细修改）。`pos` = 按全书各章原始可见文本字符权重算出的全书位置 0–1（与前端文本序同制），供 notes.md 标「全书 N%」与按位置跳转。划线不属于「正文排版」，高亮随文字流动，翻页、字号变化不丢。

划线生命周期与笔记：**备注（笔记）只写伴生 `data/library/<stem>.notes.md`，且只有写了备注的划线才落这个 md**；备注正文的真相源就是 notes.md——阅读器内点划线弹备注框保存，与外部 md 软件直接编辑，写的是同一处，程序不另做 md 编辑器。md 结构：按章分 `##` 标题（`第 N 章 · 标题（N/M）`）；每条划线 = 程序保护区 + 用户自由区。保护区 = `<!-- icedreader-note` 注释块（`key: value` 行：`id`/`color`/`created`/`deleted`/`posPct`（全书位置，整数百分比 0–100，与界面跳转/摘抄行同单位，非 0–1 小数），`-->` 结束，与元数据 md 同款注释风格，用户别碰）+ 紧跟的**摘抄行**（md 引用块、`> ` 开头，md 软件里看得舒服：`> 【重点|摘抄】摘录（全书 N% · 划于 2026-09-05 14:30）`，位置/时间由程序生成；删除划线时该行包 md 删除线并附（已删于 …））；程序按「注释块后首个以 `> ` 开头的行」识别摘抄行；注释块到下一个注释块/章标题之间的其余文字 = **用户笔记区**，程序重写 md 时只动保护区，**用户区逐字原样保留**，并读回供正文悬停浮层与划线列表显示备注。程序宽容解析：id 对不上的孤立内容（手写无块段落、误删块后的残留）原样保留、不丢不覆盖。**删除划线**：正文高亮消失（`annotations.json` 移除记录），若该条在 notes.md 里则保护区记 `deleted:` 时间、引用行加删除线，划线内容与笔记保留（md 留痕，同会计不涂改）；纯划线（没写过备注）删除即 md 无痕。同一处再次划线 = 新 id 新条目。
...
Windows 编译需要 MSVC。`scripts/dev.ps1` 会载入 vsvars；打 release 前同样要载入。用户说 build release 时：`npm run tauri -- build --no-bundle`，再用 `Copy-Item` 拷一份独立文件（禁止 Rename-Item / 硬链接）到 `target/release/IcedReader-{version}-windows-x64.exe`，`{version}` 取 `src-tauri/tauri.conf.json` 的 `version`。告诉用户这份路径；GitHub Release 上传它。不要把 `iced-reader.exe` 或 `deps/iced_reader.exe` 当发布文件。只构建绿色版独立 exe（`--no-bundle`），发布时不要跑不带 `--no-bundle` 的 `build`。不要用 `--offline` 除非依赖已经在本地。

## Tauri 命令

- `open_book` / `close_book` / `pending_book` / `list_library` / `delete_book`（`fileName` + `progressKey`：删书库内 epub，并清该书进度、划线与伴生 md 与 `<stem>.notes.md`；前端必须先弹确认框再调）
...
- `save_progress`
- `list_annotations` / `add_annotation`（`…` + `color` + `pos`，键同进度键，存 `data/annotations.json`）/ `delete_annotation`（`bookId` + `key` + `id`：正文移除，notes.md 有则打 `deleted` 留痕）
- `save_note`（`bookId` + `key` + `id` + `note`：写/改该划线在 `<stem>.notes.md` 的用户笔记区；空串 = 撤掉该备注并移除 md 条目）/ `read_notes`（`bookId` + `key`：解析 notes.md 返回 `id`→笔记正文，供悬停/列表；无则空）
- `get_font_settings` / `set_use_original_fonts` / `set_font_scale` / `install_font` / `clear_font`

## 书元数据（书名规范）

- 书架条目可编辑元数据存 `data/library/<stem>.md`（与 epub 同名伴生 md，当前 v2）。**md 由程序维护**：用户只走书架三点菜单的「编辑元数据…」面板，不手编 md；md 人类可读只是可拷贝/备份/进 git 的福利。元数据不含划线（划线仍存 `data/annotations.json`，笔记在 `<stem>.notes.md`，见约束③划线生命周期）。v1 md 缺 v2 字段时解析为空，不丢已有数据。
...
- **保存即按显示名改名**：`set_book_meta` 保存后，`data/library/` 里的 epub 与伴生 md 会按“保存后的显示名（手改或拼接）”改名——Windows 禁用作文件名清洗、同名冲突自动加 `-2`/`-3`…（同 `-N` 与原名视为同一本的既有规则）；有 `id:` 进度键的书（有 identifier）进度/划线天然不受影响，`lib:` 键的书自动把进度、划线、质量信号缓存迁到新名，不会丢。`<stem>.notes.md` 若存在随 epub+md 一并改名。改名在写 md 之前执行，失败则整次保存报错。
...
改划线时：选中一段文字松手应浮出「色板 + 划线」，默认黄（重点）、可切绿（摘抄），点划线后按所选色即时上色；再点这段（无选区单击）浮出「删除划线」与「备注」；双击已有划线中的词也应是删除而非报重叠；删除后即刻消失。写备注后 hover 该划线应浮现备注内容（尽量完整可滚动）；划线下笔后不改色不调范围（要换 = 删除重划）。删除有备注的划线：先弹提示（划线将消失，notes.md 里保留内容并记删除时间），确认后正文消失且 `<stem>.notes.md` 该条带 `deleted:` 时间与删除线；纯划线（无备注）删除后 md 无痕。A±/拉宽窗口后划线仍贴在原句上。翻走再回来、重启程序后划线仍在，且 `data/annotations.json` 已更新；`<stem>.notes.md` 可在外部 md 软件编辑，程序重写只动 `icedreader-note` 块与引用行、用户区笔记原样保留。选中文字点划线后不应翻页（翻页靠左右键 / 滚轮 / 顶栏按钮，没有点左右侧点按翻页）。顶栏「划线」应列出全书画线（按阅读顺序，带色块与备注标记），点一条跳到所在章的那一页（跨章会切章，同章直接定位），列表里删除后即时消失。
